...
# %%
data = lib.Dataset("YEAR", data_path='/workspace/data', random_state=1337, quantile_transform=True, quantile_noise=1e-3)
...
for batch in lib.iterate_minibatches(data.X_train, data.y_train, batch_size=512, 
    ...
    loss_history.append(metrics['loss'].cpu().detach())
    ...
    if trainer.step % report_frequency == 0:
        ...
        with open('./figure_data.pkl', 'wb') as f:
            pkl.dump({'loss': loss_history, 'mse': mse_history}, f)
