...
if __name__ == "__main__":
    ...
    for stage in range(opt.num_nets):
        ...
        # Train
        tr_rmse  = mse(net_ensemble, train_loader)
        if opt.cv:
            val_rmse = mse(net_ensemble, val_loader) 
            if val_rmse < best_rmse:
                best_rmse = val_rmse
                best_stage = stage

        te_rmse  = mse(net_ensemble, test_loader)

        print(f'Stage: {stage}  MSE@Tr: {tr_rmse:.5f}, MSE@Val: {val_rmse:.5f}, MSE@Te: {te_rmse:.5f}')

        loss_models[stage, 0], loss_models[stage, 1] = tr_rmse, te_rmse

    tr_rmse, te_rmse = loss_models[best_stage, 0], loss_models[best_stage, 1]
    print(f'Best validation stage: {best_stage}  MSE@Tr: {tr_rmse:.5f}, final MSE@Te: {te_rmse:.5f}')
